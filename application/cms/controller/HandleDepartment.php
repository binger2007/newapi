...
{
    //加载单位信息
    public function loadDepartment2()
    {
        $db = db('department');
        $departmentId = input('departmentId');
        // return $departmentId;
        if ($departmentId) {
            $arr = $this->getChilds(array($departmentId), 0);
            $db->where('id', $departmentId)->whereOr('p_id', 'IN', $arr);
        }
        return $db->order('sort_num asc')->select();
    }

    //获取所有子节点
    private function getChilds($p_id = array(46), $level = 0)
    {
        static  $id_arr = array();
        $id_arr = db('department')->where('p_id', 'in', $p_id)->column('id');
        if (!empty($id_arr)) {
            $level++;
            $id_arr = array_merge($id_arr, $this->getChilds($id_arr, $level));
        }
        return $id_arr;
    }
    //加载单位信息
